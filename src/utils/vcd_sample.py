...
@dataclass
class CustomSampleDecoderOnlyOutput(SampleDecoderOnlyOutput):
    thresholds: Optional[float] = None


@dataclass
class CustomSampleEncoderDecoderOutput(SampleEncoderDecoderOutput):
    thresholds: Optional[float] = None
    ...
    **model_kwargs,
    ...
    while True:
        ...
        if use_cd:
            ...
            if cd_tau is not None:
                if cd_tau > 0:
                    cd_beta = sigmoid_decayed_entropy(next_token_logits, cd_tau)
                else:
                    cd_beta = scaled_normalized_entropy(next_token_logits, -cd_tau)
            threshold = torch.log(torch.tensor(cd_beta)) + next_token_logits.max(dim=-1, keepdim=True).values

            diffs = (1 + cd_alpha) * next_token_logits - cd_alpha * next_token_logits_cd
            cd_logits = diffs.masked_fill(next_token_logits < threshold, -float('inf'))
    ...
    if return_dict_in_generate:
        if self.config.is_encoder_decoder:
            return CustomSampleEncoderDecoderOutput(
                sequences=input_ids,
                scores=scores,
            ...
            return CustomSampleDecoderOnlyOutput(
                sequences=input_ids,
                scores=scores,
