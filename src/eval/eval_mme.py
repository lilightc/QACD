...
def convert():
    args = get_args()
    GT = get_gt(data_path=args.gt_dir)
    result_dir = os.path.splitext(args.model_outputs)[0]
    os.makedirs(result_dir, exist_ok=True)
    answers = [json.loads(line) for line in open(args.model_outputs)]

    results = defaultdict(list)
    for answer in answers:
        category = answer['image'].split('/')[0]
        file = answer['image'].split('/')[-1].split('.')[0] + '.txt'
    ...
    for category, cate_tups in results.items():
        with open(os.path.join(result_dir, f'{category}.txt'), 'w') as fp:
            for file, prompt, answer in cate_tups:
                ...
                tup = file, prompt, gt_ans, answer[:4]
                fp.write('\t'.join(tup) + '\n')
    return result_dir
...
def get_args():
    ...
    parser.add_argument('--gt-dir', type=str, default='./data/MME_Benchmark_release_version/MME_Benchmark')
