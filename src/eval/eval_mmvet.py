...
def jsonl2json(jsonl):
    dic = {}
    for line in jsonl:
        qid = line['question_id']
        dic[f'v1_{qid}'] = line['text']
    return dic


def main(gt_dir, model_outputs, data_root='./data/MMVet'):
    ...
    model = os.path.splitext(model_outputs)[0]
    os.makedirs(model, exist_ok=True)
    num_run = 1 # we set it as 5 in the paper
    model_results_file = f"{model}/mmvet_results.json"

    # grade results for each sample to svae
    grade_file = f'{model}/gpt-grade-{num_run}runs.json'
    grade_file = f'{model}/gpt-grade-{num_run}runs.json'

    # score results regarding capabilities/capability integration to save
    cap_score_file = f'{model}/{sub_set_name}gpt-cap-score-{num_run}runs.csv'
    cap_int_score_file = f'{model}/{sub_set_name}gpt-cap-int-score-{num_run}runs.csv'

    results = jsonl2json([json.loads(i) for i in open(model_outputs, 'r')])
    ...
    while need_more_runs():
        for j in range(num_run):
            for id, line in tqdm(data.items()):
                ...
                messages = [
                    {"role": "system", "content": 'You are a helpful and precise assistant for checking the quality of the answer. Your only function is to return a single float between 0.0 and 1.0. Do not explain your reasoning or any words.'},
...
def get_args():
    ...
    parser.add_argument('--gt-dir', type=str, default='./data/MMVet/mm-vet.json')
    return parser.parse_args()


if __name__ == '__main__':
    args = get_args()
    main(args.gt_dir, args.model_outputs)
