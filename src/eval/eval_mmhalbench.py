...
def main(args):
    records = [json.loads(i) for i in open(args.model_outputs, 'r')]
    questions = [json.loads(i) for i in open(args.questions, 'r')]
    os.makedirs(os.path.splitext(args.model_outputs)[0], exist_ok=True)
    save_path = os.path.join(os.path.splitext(args.model_outputs)[0], 'gpt_results.json')
    assert len(records) == 96

    # ask GPT-4 to evaluate
    responses = []
    for i, (record, query) in tqdm(enumerate(zip(records, questions)), total=96, ncols=79):
        image_content = ', '.join(query['image_content'])
        input_text = template.format(image_content, query['question'], query['gt_answer'], record['text'])
...
def get_args():
    ...
    parser.add_argument("--questions", type=str, default='./data/MMHalBench/mmhalbench.jsonl')
